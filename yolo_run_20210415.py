...
parser.add_argument('--learning_rate',type = float, default=0.0000001, help = 'laerning rate')
...
def get_lr(base_lr=0.0000001, lr_decay=0.01):
    #bd = [2, 4, 8]
    bd = [16, 18, 20]
...
def load_latest_model(model, opt=None):
    if glob.glob('models/yolo_epoch*'):
        ...
        return int(latest_model.split('epoch')[-1])+1
    return  0
...
def train():
    ...
    if args.optimizer_new:
        ...
        epoch_begin = load_latest_model(model,opt)
    # opt = paddle
